...
##import data
  ...
  #these objects are called "study_normalisedbetas" and "samplepheno"
...
#NB! only 183 CpG sites are in the study dataset
#which ones are missing?
missingcpgs <-  cpgs %in% rownames(study_normalisedbetas)
...
############################################################################################################################################################
#subset study data and order
############################################################################################################################################################

#study_data_down
select <- rownames(study_normalisedbetas) %in% Illig_data_down$cpgs
study_normalisedbetas_down <- subset(study_normalisedbetas, select =="TRUE")


#study_data_up
select <- rownames(study_normalisedbetas) %in% Illig_data_up$cpgs
study_normalisedbetas_up <- subset(study_normalisedbetas, select =="TRUE")
...
#sort study data by Cpg name
study_normalisedbetas_up <- study_normalisedbetas_up[order(rownames(study_normalisedbetas_up)),]
study_normalisedbetas_down <- study_normalisedbetas_down[order(rownames(study_normalisedbetas_down)),]


head(rownames(study_normalisedbetas_up))
head(rownames(study_normalisedbetas_down))
head(Illig_data_up$cpgs)
head(Illig_data_down$cpgs)

############################################################################################################################################################
#Calculate diffs between study beta values and the reference for each CpG site - ##UP###
############################################################################################################################################################

matrix_up<- matrix(nrow=nrow(Illig_data_up), ncol=ncol(study_normalisedbetas_up))
for (i in 1:ncol(study_normalisedbetas_up)){
  matrix_up[,i]<- (study_normalisedbetas_up[,i])-(Illig_data_up$reference_never_median_beta_all)}
colnames(matrix_up)<- colnames(study_normalisedbetas_up)
...
scores_up<- as.numeric(rep(NA,ncol(study_normalisedbetas_up)))
for (i in 1:ncol(study_normalisedbetas_up)){
  scores_up[i]<-sum(matrix_up[,i]*Illig_data_up$weights)}

plot(samplepheno$smk_categoryv1, scores_up)

rm(Illig_data_up, study_normalisedbetas_up, matrix_up)
############################################################################################################################################################
#Calculate diffs between study beta values and the reference for each CpG site - ##DOWN###
############################################################################################################################################################

matrix_down<- matrix(nrow=nrow(Illig_data_down), ncol=ncol(study_normalisedbetas_down))
for (i in 1:ncol(study_normalisedbetas_down)){
  matrix_down[,i]<- (Illig_data_down$reference_never_median_beta_all)-(study_normalisedbetas_down[,i])}
colnames(matrix_down)<- colnames(study_normalisedbetas_down)
...
##calculate scores for each individual in the dataset
scores_down<- as.numeric(rep(NA,ncol(study_normalisedbetas_down)))
for (i in 1:ncol(study_normalisedbetas_down)){
  scores_down[i]<-sum(matrix_down[,i]*Illig_data_down$weights)}

plot(samplepheno$smk_categoryv1, scores_down)

rm(Illig_data_down, study_normalisedbetas_down, matrix_down)
############################################################################################################################################################
#combine scores
############################################################################################################################################################
